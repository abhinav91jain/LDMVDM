...
dataset = KittiDataset(opt.dataroot, imgSize, 3)
...
for epoch in range(max(0, opt.which_epoch), opt.epoch_num+1):
    ...
    for ii, data in enumerate(dataloader):
        ...
        if step_num%opt.print_freq == 0:
            elapsed_time = timer()-t
            print(str(epoch) + ': ' + str(step_num) + ' / ' + str(int(len(dataset)/opt.batchSize)) + ', ... elapsed time: %f (s)' % (elapsed_time))
            print(inv_depths_mean)
            t = timer()

        if step_num%opt.display_freq == 0:
            frame_vis = frames.data.permute(1,2,0).contiguous().cpu().numpy().astype(np.uint8)
            depth_vis = visDepthMap(inv_depths.data.cpu()).numpy().astype(np.uint8)
            sio.savemat(os.path.join(opt.checkpoints_dir, 'depth_' + str(step_num) + '.mat'), {'D': inv_depths.data.cpu().numpy(), 'I': frame_vis})

        if step_num%opt.save_latest_freq == 0:
            print("cache model....")
