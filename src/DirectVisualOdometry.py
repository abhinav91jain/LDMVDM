...
def sampling(A, x, y):
	...
	x1 = x/((w-1)/2) - 1
	y1 = y/((h-1)/2) - 1
	grid = torch.cat(x1.view(batch,h,w,1), y1.view(batch,h,w,1), 3)
	Q = grid_sample(A, grid, mode='bilinear')
    in_view_mask = Variable(((x1.data > -1+2/w) & (x1.data < 1-2/w) & (y1.data > -1+2/h) & (y1.data < 1-2/h)).type_as(A.data))
    return Q.view(batch, k, h*w), in_view_mask


def meshgrid(x, y):
    X = x.unsqueeze(0).repeat(y.size(0), 1)
    Y = y.unsqueeze(1).repeat(1, x.size(0))
    return X, Y
...
class Twist2Mat(nn.Module):
    ...
    def cprodmat_batch(self, a_batch):
    	o = Variable(self.o).expand(a_batch.size(), 1)
        a0 = a_batch[:, 0:1]
        a1 = a_batch[:, 1:2]
        a2 = a_batch[:, 2:3]
        return torch.cat((o, -a2, a1, a2, o, -a0, -a1, a0, o), 1).view(a_batch.size(), 3, 3)
    ...
    def compute_SSIM(img0, mu0, sigma0, img1, mu1, sigma1):
    	...
    	ssim_n = (2*mu0*mu1 + .001) * (2*sigma01 + .009)
    	ssim_d = (mu0**2 + mu1**2 + .001) * (sigma0 + sigma1 + .009)
    	ssim = ssim_n / ssim_d
...
class DirectVO(nn.Module):
    ...
    def warp_batch(self, img_batch, level_idx, R_batch, t_batch):
        ...
        _, N = xy.size()
        xyz = R_batch[:, :, 0:2].bmm(xy.view(1, 2, N).expand(batch_size, 2, N)) + R_batch[:, :, 2:3].expand(batch_size, 3, N)\
        ...
        Q, in_view_mask =  sampling(img_batch, u_warp, v_warp)
        return Q, in_view_mask * (z.view_as(in_view_mask)>1e-10).float()


    def compute_phtometric_loss(self, ref_frames_pyramid, src_frames_pyramid, ref_inv_depth_pyramid, src_inv_depth_pyramid,rot_mat_batch, trans_batch,use_ssim=True, levels=None,
                                ref_expl_mask_pyramid=None, src_expl_mask_pyramid=None):
        ...
        for level_idx in levels:
            ...
            warp_img = warp_img.view((bundle_size-1)*2, CHANNEL, h, w)
            if use_expl_mask:
                mask = in_view_mask.view(-1,h,w)*expl_mask_pyramid[level_idx]
            else:
                mask = in_view_mask
            mask_expand = mask.view((bundle_size-1)*2, 1, h, w).expand((bundle_size-1)*2, CHANNEL, h, w)
