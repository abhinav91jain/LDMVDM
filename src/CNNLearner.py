...
class CNNLearner(nn.Module):
    def __init__(self, img_size=[128, 416], ref_frame_idx=1,l1=.5, l2=0.01, use_ssim=True, smooth_term = 'lap',use_expl_mask=False, gpu_ids=[0]):
        super(CNNLearner, self).__init__()
        self.cnnkernel = nn.DataParallel(CNNKernel(img_size, smooth_term = smooth_term, use_expl_mask=use_expl_mask), device_ids=gpu_ids)
    ...
    def forward(self, frames, camparams, max_lk_iter_num=10):
        cost, photometric_cost, smoothness_cost, ref_frame, ref_inv_depth, ref_expl_mask = self.cnnkernel.forward(frames, camparams, self.ref_frame_idx,self.l1, self.l2, use_ssim=self.use_ssim)
        return cost.mean(), photometric_cost.mean(), smoothness_cost.mean(), ref_frame, ref_inv_depth, ref_expl_mask

    def save_model(self, file_path):
        torch.save(self.cpu().cnnkernel.module.depth_net.state_dict(),file_path+'_depth_net.pth')
        torch.save(self.cnnkernel.module.pose_net.state_dict(), file_path+'_pose_net.pth')
        self.cuda()

    def load_model(self, file_path):
        self.cnnkernel.module.depth_net.load_state_dict(torch.load(file_path+'_depth_net.pth'))
        self.cnnkernel.module.pose_net.load_state_dict(torch.load(file_path+'_pose_net.pth'))

    def init_weights(self):
        self.cnnkernel.module.depth_net.init_weights()

    def get_parameters(self):
        return itertools.chain(self.cnnkernel.module.depth_net.parameters(),self.cnnkernel.module.pose_net.parameters())
...
class CNNKernel(nn.Module):
    ...
    def forward(self, frames, camparams, ref_frame_idx, l1=.5, l2=.01, do_data_augment=True, use_ssim=True):
        ...
        photometric_cost = self.vo.compute_phtometric_loss(ref_frame_pyramid,src_frames_pyramid,ref_inv_depth_pyramid,src_inv_depth_pyramid,rot_mat_batch, trans_batch,
                                                levels=[0,1,2,3], use_ssim=use_ssim,ref_expl_mask_pyramid=ref_expl_mask_pyramid,src_expl_mask_pyramid=src_expl_mask_pyramid)
