    ...
    def forward(self, frames, camparams, refIdx, S=.5, do_data_augment=True, use_ssim=True, iterNum=10, level=1):
        ...
        if do_data_augment:
            if np.random.rand()>.5:
                ...
                camparams[2] = self.imgSize[1] - camparams[2]
        ...
        smoothness_cost = self.vo.multi_scale_image_aware_smoothness_cost(inv_depth0_pyramid, frames_pyramid, levels=[2,3], type=self.smoothTerm) + self.vo.multi_scale_image_aware_smoothness_cost(inv_depth_norm_pyramid, frames_pyramid, levels=[2,3], type=self.smoothTerm)
